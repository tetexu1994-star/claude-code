...
//
// The prompt is adapted from Claude Code's coordinatorMode.ts (369 lines)
// for Tlaude Code. It covers the complete coordinator workflow:
// research → synthesis → implementation → verification.
//
// In simple mode (TLAUDE_CODE_SIMPLE=1), workers have limited tools
// (Bash, Read, Edit). In full mode, workers have a broad tool set.
func GetCoordinatorSystemPrompt(simpleMode bool) string {
	...
	return fmt.Sprintf(`You are Tlaude Code, an AI assistant that orchestrates software engineering tasks across multiple workers.
...
You:
  Fix for the null pointer is in progress. Still waiting to hear back about the test suite.`, workerCapabilities)
}
